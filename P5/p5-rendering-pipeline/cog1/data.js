...
define(["exports", "data", "glMatrix"], function (data, exports) {
  ...
  /**
   * Helper function to calculate cross product of two vectors with dim 3.
   * param vector A
   * param vector B
   * returns vector product of vector A and vector B
   */
  function calcCrossProduct(vectA, vectB) {
    let crossProduct = [];

    crossProduct[0] = vectA[1] * vectB[2] - vectA[2] * vectB[1];
    crossProduct[1] = vectA[2] * vectB[0] - vectA[0] * vectB[2];
    crossProduct[2] = vectA[0] * vectB[1] - vectA[1] * vectB[0];

    return crossProduct;
  }

  /**
   * Helper function to calc dot product of two vectors with dim 3.
   * param vector A
   * param vector B
   * returns dot product of vector A and vector B
   */
  function calcDotProduct(vectA, vectB) {
    let dotProduct = 0;

    for (let i = 0; i < 3; i++) {
      dotProduct = dotProduct + vectA[i] * vectB[i];
    }
    ...
    for (var p = 0; p < this.polygonVertices.length; p++) {
      ...
      for (var v = 1; v < polygon.length - 1; v++) {
        ...
        if (this.textureURL != "" && this.polygonTextureCoord.length != 0) {
          ...
          this.trianglePolygonTextureCoord[nbTris].push(
            this.polygonTextureCoord[p][0]
          );
          this.trianglePolygonTextureCoord[nbTris].push(
            this.polygonTextureCoord[p][v]
          );
          this.trianglePolygonTextureCoord[nbTris].push(
            this.polygonTextureCoord[p][v + 1]
          );
        }

...
function(exports, scenegraph, animation) {
	 ...
	 */
	function init() {
        var uvCube = scenegraph.createNodeWithModel("uvCube", "cube", {scale: 300, textureURL: "uvTest.jpg"});
        var uvPlain = scenegraph.createNodeWithModel("uvPlain", "plain", {scale: 300, textureURL: "uvTest.jpg"});
        uvCube.setVisible(true)
        uvPlain.setVisible(false)
        return;
		...
		let cube1 = scenegraph.createNodeWithModel("cube 1", "cube",{ scale: 200 });
		cube1.translate([0,0,0]);
		cube1.rotate([0,0,0]);

		let cube2 = scenegraph.createNodeWithModel("cube 2", "cube",{ scale: 150 },cube1);
		cube2.translate([350,0,0]);
		cube2.rotate([0.75,0,0]);

		let cube3 = scenegraph.createNodeWithModel("cube 3", "cube",{ scale: 100 },cube2);
		cube3.translate([0,250,0]);
		cube3.rotate([0,0.75,0]);
        return;

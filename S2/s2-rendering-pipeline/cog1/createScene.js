...
define(["exports", "scenegraph", "animation"], //
	function (exports, scenegraph, animation) {
		...
		function init() {
			...
			var sunCenter = scenegraph.createNodeWithModel("sunCenter", "sphere", { scale: 1 });
			sunCenter.translate([-420, 216, 0]);
			animation.assign(sunCenter, 'rotate', { rotationSpeed: [0, 0.01, 0] });

			var sun = scenegraph.createNodeWithModel("sun", "sphere", { scale: 40 });
			sun.translate([-420, 216, 0]);

			var planet = scenegraph.createNodeWithModel("planet", "sphere", { scale: 20 }, sunCenter);
			planet.translate([-270, -4, -0]);

			var planetCenter = scenegraph.createNodeWithModel("planetCenter", "sphere", { scale: 1 }, sunCenter);
			planetCenter.translate([-270, -4, -0]);
			animation.assign(planetCenter, 'rotate', { rotationSpeed: [0, 0.008, 0.008] });

			var mond = scenegraph.createNodeWithModel("mond", "sphere", { scale: 5 }, planetCenter);
			mond.translate([0, 0, 50]);
